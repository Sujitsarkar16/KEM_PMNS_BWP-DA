"""
Random Forest Optimized - Clean Top 15 Features (No Data Leakage)
==================================================================

This script implements an optimized Random Forest model with comprehensive
...
- min_samples_split: Minimum samples to split (INCREASED)
- min_samples_leaf: Minimum samples per leaf (INCREASED)
...
class RFTop30Optimized:
    """
    Random Forest optimized model with hyperparameter tuning
    """
    
    def __init__(self, data_path='paper/data/clean_top15_features_20251206.csv'):
        """Initialize RF optimized implementation with CLEAN features (no data leakage)"""
    ...
    def define_hyperparameter_search_space(self):
        ...
        # UPDATED: Regularization to prevent overfitting
        param_distributions = {
            'n_estimators': [100, 200, 300, 400, 500],
            'max_depth': [3, 5, 7, 10, None],  # Reduced max depth range
            'min_samples_split': [5, 10, 15, 20],  # Increased min samples
            'min_samples_leaf': [2, 4, 6, 8],  # Increased min leaf samples
            'max_features': ['sqrt', 'log2', 0.3, 0.5],  # Reduced features per split
            'bootstrap': [True]  # Force bootstrap for generalization
        }
    ...
    def train_final_model(self, random_state=42):
        ...
        print(f"  - Trees: {self.best_model.n_estimators}")
    ...
    def save_results(self):
        ...
        output_dir = 'paper/results/rf_clean_top15'
        ...
        comprehensive_results = {
            'model_type': 'RandomForest_Clean_Top15_NoLeakage',
        ...
        # Save files
        results_file = f'{output_dir}/rf_top30_optimized_results_{timestamp}.json'
        ...
        metrics_file = f'{output_dir}/rf_top30_optimized_metrics_{timestamp}.csv'
        pd.DataFrame(metrics_data).to_csv(metrics_file, index=False)
        
        # Feature importance
        importance_file = f'{output_dir}/rf_top30_optimized_importance_{timestamp}.csv'
        self.feature_importance.to_csv(importance_file, index=False)
        
        # Search results
        search_df = pd.DataFrame(self.search_results)
        search_file = f'{output_dir}/rf_top30_search_results_{timestamp}.csv'
        search_df.to_csv(search_file, index=False)
        
        # Model
        model_file = f'{output_dir}/rf_top30_optimized_model_{timestamp}.pkl'
...
def main():
    """Main function"""
    print("=" * 80)
    print("RANDOM FOREST OPTIMIZED - TOP 30 ENGINEERED FEATURES")
    print("=" * 80)
